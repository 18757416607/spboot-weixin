...
public class BankServiceImpl implements BankService{
     ...
     *      checkcode : 手机验证码
     * @return
     *      -1：系统报错   00：成功  01：参数为空  02：表更新不成功   03：验证码失效,请重新获取验证码!  04:验证码错误!   05:银联返回的消息 06:换绑时银行卡号一样
     * 说明：
     *    若该车牌号已有绑定银行卡就是先解绑 在 绑定
     * @throws Exception
     */
    @Transactional
    public Result bindBankCard(Map<String,Object> param, HttpServletRequest req, HttpServletResponse resp) throws Exception{
        ...
        param.put("bind_username",param.get("username"));  //给调用银联接口使用
        param.put("bind_name",param.get("name"));          //给调用银联接口使用

        boolean mark = false;  //用来表示 绑定还是换绑
        //如果已有绑定银行卡,就先解绑 在进行绑定
        ...
        if(count>0){
            ...
            if(result.getCode().equals("00")){
                ...
                if(count1>0){
                    ...
                    if(mark){
                        logger.info("小程序绑定银行卡-->换绑成功");
                        return ResultUtil.requestSuccess("换绑成功","换绑成功");
                    }
                    logger.info("小程序绑定银行卡-->绑卡成功");
                    return ResultUtil.requestSuccess("绑卡成功","绑卡成功");
                    /*if(count2>0){
                        logger.info("小程序绑定银行卡-->绑卡成功");
