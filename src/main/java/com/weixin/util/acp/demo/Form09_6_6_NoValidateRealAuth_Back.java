package com.weixin.util.acp.demo;
...
public class Form09_6_6_NoValidateRealAuth_Back extends HttpServlet {
	...
	private static Map<String, String> setFormDate() {
		...
		customerInfoMap.put("customerNm", "马秋梁");					//姓名
		customerInfoMap.put("phoneNo", "15268034063");			    //手机号
		...
		data.put("bindId", uuid);       //【本交易中bindId必送】可以自行定义 1-128位字母、数字和/或特殊符号字符,代收使用Form09_6_2_DaiShou_BindId.java
		System.out.println("bindId"+uuid);

		//////////如果商户号开通了【商户对敏感信息加密】的权限那么需要对 accNo，pin和phoneNo，cvn2，expired加密（如果这些上送的话）对敏感信息加密使用：
		String accNo = AcpService.encryptData("6221560477435971", "UTF-8");  //【贷记卡】这里测试的时候使用的是测试卡号，正式环境请使用真实卡号
